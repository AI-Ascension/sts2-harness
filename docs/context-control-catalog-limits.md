...
historical contract-origin revision. Consumer parity and pin adoption remain separate work.

Other limits must not be collapsed into these five fields:

- `context_control/types.rs` bounds individual note bytes at 4096. This is absent from the
  five-field catalog and is not newly advertised here.
...
- Pins must be selected. Reference integrity, expiry, protected content and UTF-8 validity
  are independent checks. No descriptor relation between objective and whole-context ceilings
  is introduced; concrete final serialization must fit independently.
