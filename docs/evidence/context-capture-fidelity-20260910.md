...
attempt IDs are generated for every invocation, and lifecycle records receive a distinct event
identity with an explicit parent link; the parent remains unavailable when no upstream snapshot is
supplied. Capture snapshot IDs use length-prefixed hashing so hyphenated execution and attempt IDs
cannot collide.
