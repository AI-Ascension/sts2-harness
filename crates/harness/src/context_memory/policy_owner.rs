...
impl MemoryPolicyOwner {
    pub fn open(
        ...
        Ok(Self { authority, store: Mutex::new(store) })
    }

    /// Only trusted test composition can inject a persistence failure.
    pub fn set_failpoint(&self, point: Option<PolicyStoreFailpoint>) -> Result<(), PolicyOwnerError> {
        self.store.lock().map_err(|_| PolicyOwnerError::Unavailable)?.failpoint = point;
        Ok(())
    }
    ...
    pub fn execute(
        ...
        command.validate()?;
        self.authority.with_authorized(access, command.permission(), |state, actor, clock| {
            let mut store = self.store.lock().map_err(|_| PolicyOwnerError::Unavailable)?;
            store.change(|journal| {
    ...
    pub fn active_binding(
        ...
        self.read(access, PolicyPermission::ReadMetadata, |journal| Ok(journal.active.clone()))
    }
    ...
    pub fn lookup_receipt(
        ...
        self.authority.with_authorized(access, PolicyPermission::ReadMetadata, |_, actor, _| {
            let store = self.store.lock().map_err(|_| PolicyOwnerError::Unavailable)?;
            let journal = store.load()?;
            journal.receipts.iter()
                .find(|receipt| receipt.subject == actor.subject && receipt.idempotency_key == key)
                .cloned().ok_or(PolicyOwnerError::Missing)
        })
    }
    ...
    fn read<T>(
        ...
        self.authority.with_authorized(access, permission, |_, _, _| {
            let store = self.store.lock().map_err(|_| PolicyOwnerError::Unavailable)?;
            action(&store.load()?)
        })
    }
