...
use sts2_harness::{
    ...
    BranchContinuationSelector, BranchContinuationStrategyPlan, BranchFork, BranchStrategy,
    DurableBranchDraft, DurableBranchStatus, ExactArtifactStore, ExactStateDigest, OccurrenceId,
    SqliteBranchStore,
};
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        error.contains("use explicit resume with verified-boundary and current-owner evidence")
    );
...
include!("branch_continuation_runtime_owner_claim_tests.rs");
