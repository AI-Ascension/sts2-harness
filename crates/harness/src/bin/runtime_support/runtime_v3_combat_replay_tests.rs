...
mod tests {
    ...
    #[test]
    fn configured_replay_path_is_loaded_with_digest_terminal_evidence() {
        let path = std::env::temp_dir().join(format!(
            "sts2-harness-combat-replay-{}.jsonl",
            std::process::id()
        ));
        let terminal = json!({
            "event": "combat_demo_complete",
            "terminal_observation_digest": "0".repeat(64)
        });
        let source = format!("{}\n{}\n", json!({"event": "model_decision"}), terminal);
        let write = std::fs::write(&path, source);
        let replay = write.and_then(|_| Replay::load(Some(&path)).map_err(std::io::Error::other));
    ...
    #[test]
    fn captured_replay_bytes_are_used_after_the_selected_file_is_replaced() {
        let path = std::env::temp_dir().join(format!(
            "sts2-harness-combat-replay-captured-{}.jsonl",
            std::process::id()
        ));
        let terminal = json!({
            "event": "combat_demo_complete",
        ...
        let source = format!("{}\n{}\n", json!({"event": "model_decision"}), terminal);
        std::fs::write(&path, &source).expect("write replay fixture");
        let captured = super::super::super::workflow_binding::read_replay_source(&path)
            .expect("capture replay bytes");
        std::fs::write(&path, b"not-json\n").expect("replace replay fixture");
        let replay = Replay::load_bytes(&captured).expect("load captured replay bytes");
        let _ = std::fs::remove_file(&path);
