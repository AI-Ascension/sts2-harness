...
impl Replay {
    pub(super) fn load(path: Option<&Path>) -> Result<Self, String> {
        ...
        let bytes = super::super::workflow_binding::read_replay_source(path)?;
        Self::load_bytes(&bytes)
    }

    pub(super) fn load_bytes(bytes: &[u8]) -> Result<Self, String> {
        if bytes.len() > 8 * 1024 * 1024 {
            return Err("replay trajectory exceeds bound".into());
        }
        let text = std::str::from_utf8(bytes).map_err(|_| "invalid replay JSON")?;
        let values = text
            .lines()
