...
use super::types::{
    AttemptKind, Checkpoint, ExecutionFingerprint, ExecutionLineage, MAX_CATALOG_BYTES,
...
impl ExecutionStore {
    ...
    pub fn reconstruct_attempt(
        ...
        if stored_checkpoint.0 != checkpoint.state_id
            ...
            || stored_checkpoint.9 != checkpoint.catalog_raw
        {
            return Err(super::types::ExecutionStoreError::InvalidCheckpoint);
        }
        tx.execute(
        ...
        let copied = tx
            .execute(
             ...
             SELECT episode_id, ?2, sequence, state_id, generation, seed, build_digest,
                    ...
                    legal_actions_digest, legal_actions_raw, ?3
