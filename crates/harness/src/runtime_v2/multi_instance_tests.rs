...
mod multi_instance_tests {
    ...
    #[test]
    fn global_and_instance_capacity_never_forward_overload() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[test]
    fn queued_cancel_and_shutdown_leave_active_work_explicit() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(report.active_operations(), &[dispatched.operation_id().clone()]);
        assert_eq!(coordinator.admit(active), Err(RuntimeV2CoordinatorError::AdmissionClosed));
