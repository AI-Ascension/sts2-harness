...
/// Production seam with a call counter so the ordering claim is observable.
#[derive(Default)]
struct CountingSeam {
    calls: Cell<u32>,
}
...
fn refusal_returns_before_the_allocation_seam_is_invoked() {
    ...
    // The same declarations then reach the seam exactly once.
    let seam = CountingSeam::default();
    admit_and_allocate(&incumbent, &parse(&document()), &seam).unwrap();
    assert_eq!(seam.calls(), 1);
}
